...
def train(net,criterion,optimizer,batch_size,train_samples,epoch,loss_history=None,beta=None,method=None,first_epoch=True,phi='KL',importance_weights=0,speedUp=1):
    ...
    if first_epoch:
        ...
        for i,data in enumerate(train_dl):
            x,y = data
            train_features = torch.tensor(x, dtype=torch.float32, requires_grad=True).reshape(-1, 1, 599).to(device)
            train_labels = torch.tensor(y, dtype=torch.float32, requires_grad=True).reshape(-1, 1).to(device)
            prediction = net(train_features)
            loss = nn.MSELoss(reduction='mean')(prediction,train_labels)
            optimizer.zero_grad()
            loss.backward()  
            optimizer.step()
            if method == 'each':
                l = nn.MSELoss(reduction='none')(prediction,train_labels).cpu().data.numpy()
            else:
        ...
        for j in range(len(loss_history)):
            ...
            if method == 'each':
                ...
                batch_index = np.random.choice(list(range(q_batch_len)), size=batch_size, replace=False, p=q_batch)
            else:
                batch_index = np.random.choice(list(range(len(q_batch))), size=1, replace=True, p=q_batch)[0]

            samples_len = len(train_samples)
            #print(batch_index)
            #print(f'start:{start}, end:{end}, batch_index:{batch_index}, q_batch:{q_batch[batch_index]}')

            if method == 'each':
                x = train_features[batch_index].to(device)
                y = train_labels[batch_index].to(device)
            else:
                start = batch_index * batch_size
                end = (start + batch_size) if start + batch_size < samples_len else samples_len
                x = train_features[start:end].to(device)
            ...
            if method == 'mean':
                ...
                for i, new_value in zip(batch_index, nn.MSELoss(reduction='none')(prediction,y).cpu().data.numpy()):
                    loss_history[i] = new_value
...
def train_SGD(net,criterion,optimizer,batch_size,train_samples,epoch):
    ...
    for i, data in enumerate(train_dl):
        ...
        #print(f'{i + 1}minibatch train loss: {loss.item()}')
        #print(
        #    f'mean prediction:{prediction.cpu().data.numpy().mean()}  mean actual:{train_labels.cpu().data.numpy().mean()}')
    return net, loss_history
if __name__ == '__main__':
    ...
    parser.add_argument('-su', '--speedUp', type=int, default=20, help='speedUp parameter')
    parser.add_argument('--lr', type=float, default=0.001, help='learning parameter')
    ...
    writer = SummaryWriter('./runs/REDD/'+mark)
    ...
    if not continue_train:
        ...
        loss_history = handle_loss_history(loss_history,method=method)
        if alpha != -1:
            beta = alpha / handle_loss_history(loss_history, method).mean()
    ...
    for epoch in range(epoch,epochs):
        ...
        if epoch % 1 == 0:
            ...
            if max_EA <= EA:
                ...
                print(
                    f'duration:{duration}s, epoch{epoch+2}: recall:{recall}, precision:{precision}, f1:{f1}, accuracy:{accuracy}, EA:{EA}, SAE:{SAE}, MAE:{MAE}')
            if epoch == epochs-1:
                end_time = time.time()
